...
# -> Fill config_rel_best_*.yaml using logged results in XX_REL_CARD.md 

# Generate CARD with best configs
for FORMAT in $(ls data/gold_rel_*.jsonl_corr | cut -d_ -f 3 | cut -d. -f1); 
do patentcity eval relationship-model data/gold_rel_${FORMAT}.jsonl configs/config_rel_best_${FORMAT}.yaml >> doc/XX_REL_CARD.md; 
done;
```