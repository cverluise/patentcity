...
GEOC_OUTCOLS = ['recId', 'seqNumber', 'seqLength',
                'latitude', 'longitude', 'locationLabel', 'addressLines', 'street',
                'houseNumber', 'building', 'subdistrict', 'district',
                'city', 'postalCode', 'county', 'state', 'country',
                'relevance', 'matchType', 'matchCode', 'matchLevel',
                'matchQualityStreet', 'matchQualityHouseNumber', 'matchQualityBuilding',
                'matchQualityDistrict', 'matchQualityCity', 'matchQualityPostalCode',
                'matchQualityCounty', 'matchQualityState', 'matchQualityCountry']
...
def get_parsed_loc(loc, endpoint, debug):
    ...
    if debug and response.status_code != 200:
        typer.secho(f"{loc} failed with status code {response.status_code}",
                    fg=typer.colors.RED)
...
def post_geoc_data(data: str,
    ...
    def check_post(response):
        ...
        if Status == "accepted":
            typer.secho(f"{ok}\t{log_msg}",
                        fg=typer.colors.GREEN)
        else:
            typer.secho(f"{not_ok}\t{log_msg}",
                        fg=typer.colors.RED)
    ...
    response = requests.post(GEOC_URL,
                             headers=headers,
                             params=params,
                             data=data)
    if response.status_code == 200:
        ...
        typer.secho(f"{not_ok}Failed with status {response.status_code}\n{response.content}",
                    fg=typer.colors.RED)


@app.command()
def get_geoc_status(request_id: str,
                    api_key: str,
                    freq: int = 5,
                    verbose=False):
    ...
    def summarize_status(response, verbose):
        ...
        typer.secho(f"{now}: {ProcessedCount}/{TotalCount} ({PendingCount} pending)",
                    fg=typer.colors.BLUE)
        if int(SuccessCount) > 0:
            typer.secho(f"{ok}{SuccessCount} addresses successfully geocoded",
                        fg=typer.colors.GREEN)
        if int(ErrorCount) > 0:
    ...
    completed = False
    while not completed:
        response = requests.get(f'{GEOC_URL}/{request_id}',
                                params=params)
...
@app.command()
def get_geoc_data(request_id: str,
                  api_key: str,
                  output_dir: str = None,
                  unzip: bool = True):
    ...
    response = requests.get(f'https://batch.geocoder.ls.hereapi.com/6.2/jobs/{request_id}/result/',
                            params=params)
    if response.status_code == 200:
        ...
        for i, line in enumerate(csv_reader):
            if i > 0:  # skip header
                ...
                if int(line["seqNumber"]) > 1:
                    ...
                    if line.get("SeqNumber"):  # causes conflict in BQ which is NOT case-sensitive
                        line.pop("SeqNumber")
                    typer.echo(json.dumps({"publication_number": publication_number,
                                           "geoc": line}))
